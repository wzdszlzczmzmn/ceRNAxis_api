...
class CustomListQueryTaskNetworkView(APIView):
    ...
    ALLOWED_RNA_TYPES = ALLOWED_RNA_TYPES
    ...
    def build_network(self, task, input_items):
        ...
        input_mirna_name_map = {
            normalize_rna_name(item["name"]): item["name"]
            for item in input_items
            if item["type"] == "miRNA"
        }
    ...
    def add_rna_interaction_edges(
        ...
        for interaction in interactions:
            ...
            edge_pair_key_res = edge_pair_key(source_key, target_key)

            if edge_pair_key_res not in edge_pair_index:
                edge_pair_index[edge_pair_key_res] = edge_id
    ...
    def add_immune_annotation_edges(
        ...
        with immune_file_path.open("r", encoding="utf-8-sig", newline="") as f:
            ...
            for row in reader:
                ...
                normalized_mirna_name = normalize_rna_name(mirna_name)
                ...
                if edge_pair_key_res in edge_pair_index:
                    existing_edge_id = edge_pair_index[edge_pair_key_res]
                    append_immune_annotation_to_edge(
...
class WorkflowOutputNetworkBaseView(
    ...
    def get_extra_metadata(self, task) -> dict:
        return {}

    def get(self, request, *args, **kwargs):
        try:
            if self.task_model is None:
                raise RuntimeError("Missing task_model for workflow network view.")

            if not self.task_type:
                raise RuntimeError("Missing task_type for workflow network view.")
            ...
            result = attach_task_metadata(
                ...
                extra=self.get_extra_metadata(task),
            )
    ...
    def build_network(self, task) -> dict:
        ...
        return self.build_network_from_files(
            ceRNA_axis_file=ceRNA_axis_file,
            immune_axis_file=immune_axis_file,
        )


class PairedCohortTaskNetworkView(WorkflowOutputNetworkBaseView):
...
class HybridReferenceTaskNetworkView(WorkflowOutputNetworkBaseView):
