...
from analysis.services.axis_final import import_module2_axis_final_projects


class Command(BaseCommand):
    help = "Import Module2 TCGA axis_final results into database."

    def add_arguments(self, parser):
        parser.add_argument(
            "--root",
            required=True,
            help="Module2 result root directory.",
        )
    ...
    def handle(self, *args, **options):
        result = import_module2_axis_final_projects(
            module2_root_dir=options["root"],
        ...
        if result["success"]:
            self.stdout.write(
                self.style.SUCCESS(
                    "Module2 axis_final import finished."
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR(
                    "Module2 axis_final import finished with errors."
                )
            )

        self.stdout.write(f"root: {result['module2_root_dir']}")
        self.stdout.write(f"source: {result['source']}")
        self.stdout.write(f"module: {result['module']}")
        self.stdout.write(f"group_type: {result['group_type']}")
        self.stdout.write(f"project_dir_count: {result['project_dir_count']}")
